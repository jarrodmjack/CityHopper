  ...
  return (
    <div className="flex flex-col py-10">
      ...
      {previousSearches.length > 0 &&
        [...previousSearches]
          .reverse()
          .map((previousSearch: any, i: number) => (
            <div key={previousSearch.id} className="flex items-center gap-2">
              {previousSearch.properties.length > 0 && (
                <>
                  <span>
                    <FaSearch />
                  </span>
                  <span
                    onClick={() =>
                      handleShowPreviousSearch(previousSearch.properties)
                    }
                    key={previousSearch.id}
                    className="mb-2 flex cursor-pointer items-center gap-2 rounded-lg p-2 hover:bg-slate-100 hover:text-black"
                  >
